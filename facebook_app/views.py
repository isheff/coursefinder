...
from facebook_login import LoginHandler, Facebook, FACEBOOK_APP_ID,FACEBOOK_APP_SECRET
...
def display_institution(request, institution_id):
	...
	if attends_institution(user, institute):
		...
		response = render_to_response('facebook_app/display_institution.html', {'current_user':user, 'facebook_app_id':FACEBOOK_APP_ID, 'institution':institute, 'courses':courses})
		for key in user.login_handler.headers:
			response[key]=user.login_handler.headers[key]
		return response
	else:
...
def display_course(request, course_id):
	...
	if attends_institution(user, p.institution): # The page should only be displayed if this User attends this Course's institution
		...
		response = render_to_response('facebook_app/display_course.html', pass_to_template)
		for key in user.login_handler.headers:
			response[key]=user.login_handler.headers[key]
		return response
	else:
...
def Interest_list(request, user_id):
	...
	response = render_to_response('facebook_app/interest_list.html', pass_to_template)
	for key in user.login_handler.headers:
		response[key]=user.login_handler.headers[key]
	return response
...
def get_current_user(request):
	...
	login_handler.initialize(request)
	user = login_handler.user
	user.login_handler = login_handler
	return user
